...
class Player{
    ...
    vector<TrainSquare*>  ownedTrains;      //vector that hold a pointer to owned train squar
    vector<sf::Shape*> assets;             //vector that hold a pointer to owned assets 
    ...
    bool can_build_balance(const string& ,  vector<StreetSquare*>& );

    int houses_in_owned();

    int hotels_in_owned();

    void add_asset(sf::Shape*);
