...
sf::Vector2f GameManager::calc_player_position(Player* player, Square& square) {
...
void GameManager::move_player_idx(Player* player, size_t index){
    ...
    // Draw the player sprite at the new position
    obj.window->draw(player->playerSprite); // Use obj to access the window
...
void GameManager::handle_street_landing(Player* player, StreetSquare& street){
    ...
    cout << "The amount of money you have is :" << player->get_money() <<endl;
        ...
        if(street.get_owner() == player){
            ...
            vector<StreetSquare*>& allStreets = board->get_allStreets();
...
void GameManager::handle_train_landing(Player* player, TrainSquare& train){
...
void GameManager::handle_company_square(Player* player, CompanySquare& company, int diceRoll){
...
void GameManager::handle_player_landing(Player* player, Square& square, int diceRoll){
    ...
    if(square.get_type() == "Community Chest Square"){
        cout<< player->get_name() <<" land in free Community Chest Square .\n You earned 50 NIS !";
        player->add_money(50);
    }
...
void GameManager::buy_company(Player*  buyer, CompanySquare& company){
...
//Player buy an house
void GameManager::build_house(Player* buyer, StreetSquare& street,vector<StreetSquare*>& allStreets){
    ...
    //Create the house, add it to the owner assets & dispay on board
    obj.create_house(buyer, street);
...
//Player buy an hotel
void GameManager::build_hotel(Player* buyer, StreetSquare& street,vector<StreetSquare*>& allStreets){
    ...
    //Create the hotel, add it to the owner assets & dispay on board
    obj.create_hotel(buyer, street);
...
void GameManager::set_jail_state(Player* player, bool state){
...
void GameManager::handle_nankruptcy(Player* player){
...
void GameManager::handle_nankruptcy_(Player* bankruptPlayer, Player* player){
...
void GameManager::announce_winner(Player* player){
...
void GameManager::initialize_players(int numOfPlayers, sf::RenderWindow& window){
    ...
    Player* player = new Player(name, 1500);  // Dynamically allocate a Player object
